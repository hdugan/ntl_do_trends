...
are physically implausible. The fig*.R scripts then read the cleaned
`data/` and write their PNG(s) to `figures/`.
...
| `02_clean_profiles.R` | (no figure) — removes unrealistic bottom-point jumps in `data/profiles.csv` (temp up >5°C or DO up >20 %sat at the single deepest reading vs the next-shallowest); overwrites `data/profiles.csv` in place and logs removed points to `data/profiles_removed_points.csv` |
